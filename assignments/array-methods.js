...
ticketPriceTotal.push(
  runners.reduce((a, b) => ({ donation: a.donation + b.donation }))
);
console.log(ticketPriceTotal);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce() I want you to think of
// potential problems you could solve given the data set and the 5k fun run theme.
// Try to create and then solve 3 unique problems using one or many of the array methods listed above.

// Problem 1 using .filter()
// All donations over 200 units qualify for a unique jersey. The following array lists information on each qualifying donor.
let goldMember = runners.filter(runner => runner.donation >= 200);
console.log(goldMember);

// Problem 2 using .forEach()
// Create a congratulatory email list
let emailList = [];
runners.forEach(runner => emailList.push(runner.email));
console.log(emailList);

// Problem 3 .filter()
// The unique jersey for Gold Members was accidentally not printed in a small size.
// Create a list of relevant donors so we can contact them.
let goldestMembers = runners.filter(
  smRunner => smRunner.shirt_size === "S" && smRunner.donation >= 200
);
console.log(goldestMembers);
